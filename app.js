...
function bootstrap() {
  ...
  const TEMPLATES = {
    ...
    vep: {
      ...
      alcance: [
        ...
        "Informe/constancia de VEP (tasas no incluidas).",
      ],
    ...
    empadronamiento: {
      ...
      description:
        "Empadronamiento del inmueble ubicado en [Localidad], Departamento [Departamento], Provincia de Córdoba. Incluye el relevamiento en campo, la confección del plano de mensura correspondiente y la gestión de inscripción catastral ante la Dirección General de Catastro de la Provincia.",
      alcance: [
        "Relevamiento y medición en el lugar, con identificación de límites materiales existentes, ocupación, referencias del entorno y demás elementos necesarios para la correcta confección del plano.",
        "Confección del Plano de Mensura de Posesión con determinación de superficie, medidas lineales y angulares, colindancias, ubicación y demás datos técnicos correspondientes al inmueble.",
        "Presentación y seguimiento del expediente ante Catastro Córdoba hasta obtener la inscripción catastral del inmueble y la asignación del número de cuenta.",
      ],
    },
    fraccionamientoDerechosPosesorios: {
      name: "FRACCIONAMIENTO DE DERECHOS POSESORIOS",
      description:
        "Fraccionamiento de derechos posesorios sobre el inmueble ubicado en [Localidad], Departamento [Departamento], Provincia de Córdoba. El trabajo tiene por objeto la determinación y división de las fracciones en posesión mediante medición en campo y confección del plano correspondiente, el cual servirá de instrumento técnico para la formalización privada entre las partes.",
      alcance: [
        "Relevamiento y medición en el lugar, con identificación de límites materiales existentes, ocupación, referencias del entorno y colindancias necesarias para la correcta confección del plano de fraccionamiento.",
        "Confección del Plano de Fraccionamiento con la división de las fracciones posesorias, determinando superficie, medidas lineales y angulares, colindancias y demás datos técnicos de cada fracción resultante.",
        "El plano confeccionado tiene carácter técnico-privado y no implica la inscripción de las fracciones ante la Dirección General de Catastro ni el Registro de la Propiedad. La regularización dominial y/o catastral de cada fracción queda fuera del presente presupuesto.",
      ],
    },
  ...
  const DEFAULT_HITOS = {
    generic: [
      { d: "Confirmar día de medición", m: 50000 },
      { d: "Día de medición", m: "" },
    ],
    usucapion: [
      { d: "Confirmar día de medición", m: 50000 },
      { d: "Día de medición", m: "" },
      { d: "Visado de Colegio de Ingenieros", m: "" },
      { d: "Presentación en Catastro", m: "" },
    ],
  };
  ...
  function setupPago() {
    const renderDefaultHitos = () => {
      state.pago.hitosContainer.innerHTML = "";
      const templateKey = state.modelo.value === "usucapion" ? "usucapion" : "generic";
      (DEFAULT_HITOS[templateKey] || DEFAULT_HITOS.generic).forEach((hito) => {
        addHito(hito.d, hito.m);
      });
      updatePagoUI();
    };
    ...
    state.modelo.addEventListener("change", renderDefaultHitos);
    // Visado
